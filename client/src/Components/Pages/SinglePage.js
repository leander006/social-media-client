...
function SinglePage() {
  ...
  useEffect(() => {
    const getPost = async() =>{
      try {
        const {data} = await axiosInstance.get("/post/"+postId,config)
        setPost(data)
        setLoading(true)
      } catch (error) {
    ...
    getPost()
          ...
          const handleLikes =async(e) =>{
            ...
            try {
              dispatch(loginStart())
                  const {data}=await axiosInstance.put(`/post/likePost/${postId}`,{},config)
      ...
      const handleSaved =async(e) =>{
            ...
            try {
                  dispatch(loginStart())
                  const {data}=await axiosInstance.put(`/post/bookmarkPost/${postId}`,{},config)
                  dispatch(loginSuccess(data))
            } catch (error) {
                  dispatch(loginError())
                  console.log(error?.response?.data);
      ...
      const handleDelete =async(e) =>{
        ...
        try {
              dispatch(postStart())
              const {data}=await axiosInstance.delete(`/post/delete/${postId}`,config)
      ...
      useEffect(() => {
        const getCommet = async() =>{
          try {
            ...
            const {data} = await axiosInstance.get("/comment/allComment/"+postId,config)
      ...
      const handleComment =async(e) =>{
        ...
        try {
              dispatch(commentStart())
              const {data}=await axiosInstance.post(`/comment/${postId}`,{content:comment},config)
