...
function Edit() {
      ...
      const config ={
          ...
          useEffect(() => {
            const getPost = async() =>{
              try {
                setLoading(true)
                const {data} = await axiosInstance.get("/user/"+editId,config)
                setUser(data.user)
                setLoading(false)
              } catch (error) {
      ...
      const uploadImage = async(base64EncodedImage) => {
            try {
                  setLoad(true)
                  const {data}= await axiosInstance.post('/user/upload',{data: base64EncodedImage }
                ,config);
                setFileInputState('');
                setPreviewSource('');
                setProfile(data.url)
        ...
        const handleSubmit = async(e) =>{
            ...
            try {
                  ...
                  const user = await axiosInstance.put("/user/update/"+editId,{username:username, bio:bio, name:name,
