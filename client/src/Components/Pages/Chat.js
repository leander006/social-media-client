...
function Chat() {
  ...
  useEffect(() => {
    const getMessage = async() =>{
      try {
        ...
        const {data} = await axiosInstance.get("/message/get/"+currentChat._id,config)
  ...
  const sendMessage = async(e) =>{
        ...
        try {
            dispatch(messageStart())
            const {data} = await axiosInstance.post("/message/"+currentChat._id,{content:message},config)
  ...
  const handleDelete =async(me) =>{
    try {
          dispatch(messageStart())
          await axiosInstance.delete(`/message/delete/${me._id}`,config)
...
const groupDelete = async(e)=>{
    ...
    try {
          dispatch(chatStart());
          await axiosInstance.delete("/chat/delete/"+currentChat._id,config)
...
const handleSearch= async(query)=>{
    ...
    try {
        const {data} = await axiosInstance.get("/user/freind/search?name="+groupSearch,config)
        setAddUser(data)
    } catch (error) {
...
const handleSearched = async(query)=>{
  ...
  try {
      const {data} = await axiosInstance.get("/user/freind/search?name="+search,config)
      setSearched(data)
  } catch (error) {
      toast.error(error.response.data.error)          
  }
...
const handleRemove =async(deleteUser) =>{
  try {
      const {data} = await axiosInstance.put("/chat/remove/"+currentChat._id,{userId:deleteUser._id},config)
...
const exit = async(removeUser) =>{
  try {
    dispatch(chatStart())
    const {data}=await axiosInstance.put("/chat/remove/"+currentChat._id,{userId:removeUser._id},config)
    dispatch(chatSuccess((allChat.filter((c)  => c._id !== currentChat._id))))
    dispatch(setCurrentChat(""))
...
const handleAdd =async(addUser) =>{
  ...
  try {
    if(Users.includes(addUser._id)){
      ...
      const {data} = await axiosInstance.put("/chat/add/"+currentChat._id,{userId:addUser._id},config)
      dispatch(setCurrentChat(data));
...
const handleRename =async(e) =>{
  e.preventDefault()
  try {
      const {data} = await axiosInstance.put("/chat/rename/"+currentChat._id,{chatname:chatname},config)
      dispatch(setCurrentChat(data));
...
const create = async(e)=>{
  ...
  try {
    dispatch(chatStart())
    const {data} = await axiosInstance.post("/chat",{name:name,
